function showLinkTooltip() {
    ...
    linkTooltip.setAttribute('style', `left: 0px; top: 0px;max-width: ${circleRadius * 2}px;z-index: 99999; font-size: 13.5px;opacity: 0.0; align-items: center;display:inline-block !important; position: absolute !important;background: ${typeOfMenu == 'link-menu' ? linkSegmentColor : typeOfMenu == 'image-menu' ? imageSegmentColor : regularSegmentColor}; border-radius: 9px;color: white; text-align: center; padding: 6px; pointer-events: none;`);
    ...
    label.innerHTML = typeOfMenu == 'link-menu' ?
        // hoveredLinkTitle !== null && hoveredLinkTitle !== hoveredLink && hoveredLinkTitle !== hoveredLink.replaceAll('http://', '') ? (hoveredLinkTitle.length > 24 ? hoveredLinkTitle.substring(0, 24) + '...' : hoveredLinkTitle)
        //     : 
